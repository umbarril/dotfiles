...
awww img "$FULL_PATH" --transition-fps 60 --transition-type grow

# wait a bit for transition to happen..
sleep 1.0

wal --cols16 darken -i "$FULL_PATH"  # reload pywal colors
killall waybar; setsid waybar # and reload everything that uses pywal colors vvvv
